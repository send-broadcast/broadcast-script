...
### Added
- `./broadcast.sh fix` repairs installation drift idempotently: recreates missing directories, restores ownership and executable bits, the sudoers entry and docker group membership, installs/enables/starts missing systemd units (broadcast, post-upgrade cleanup, logs watcher), re-adds missing cron entries without touching existing ones, and generates absent encryption keys — reporting ok:/fixed: per check. Missing one-time prerequisites (like Docker itself) are reported as unfixable with exit 1 and point at a full install instead of risking a half-reinstall.
