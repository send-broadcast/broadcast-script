...
HARNESS_SCRIPTS="common.sh preflight.sh restore.sh backup.sh update.sh upgrade.sh downgrade.sh trigger.sh monitor.sh diagnose.sh fix.sh health.sh recover.sh init-services.sh restart.sh"
...
harness_make_sandbox() {
    ...
    for s in $HARNESS_SCRIPTS install.sh logs.sh start.sh stop.sh restart.sh; do
        sed -e "s|/opt/broadcast|$SANDBOX_ROOT|g" \
            ...
            -e "s|/var/run|$SANDBOX_ROOT/var/run|g" \
