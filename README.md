...
- **`upgrade edge`** — **Developers only.** Install the unreleased build of the current `main` branch instead of a release. A new `edge` image is published on demand (a manually triggered GitHub Actions build of `main`), replacing the previous one; re-run `upgrade edge` any time to pull the newest. The app shows which build is running next to its version, e.g. `2.31.1 (edge-cb53118 2026-08-18)`. Edge builds may contain database migrations that are in no release, and migrations do not roll back — once a server has run edge, going back to a release against the same database is not supported. Use it on development or throwaway servers only, never on a production install; the command asks you to confirm this, and automated (cron-driven) upgrades refuse the tag unless the host has opted in.
- **`edge-enable [--no-restart]`** / **`edge-disable [--no-restart]`** — One-command opt-in/out of the edge channel. `edge-enable` sets both gates: the host marker that lets the automated upgrade path accept `edge`, and `BROADCAST_EDGE_ENABLED=1` in `app/.env` so the Broadcast UI shows its "Developer builds" card. It restarts the app so the env change applies immediately; pass `--no-restart` to defer that. `edge-disable` reverses both. A normal install shows no edge UI and accepts no edge trigger.
