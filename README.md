...
- **`edge-enable`** / **`edge-disable`** — Opt this host in or out of dashboard-triggered edge upgrades. The Broadcast web UI can offer a "Switch to edge build" card (when `BROADCAST_EDGE_ENABLED=1` is set in `app/.env`), and its trigger arrives through the automated upgrade path — which refuses `edge` unless `edge-enable` has been run on the host. Both switches are deliberate: a normal install shows no edge UI and accepts no edge trigger.
